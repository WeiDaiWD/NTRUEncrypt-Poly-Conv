...
void swap(void *x,void *y,int bytes,int mask)
{
  ...
  while (bytes >= 32) {
    __m256i xi = _mm256_loadu_si256((const __m256i*)x);
    __m256i yi = _mm256_loadu_si256((const __m256i*)y);
    __m256i xinew = _mm256_blendv_epi8(xi,yi,maskvec);
    __m256i yinew = _mm256_blendv_epi8(yi,xi,maskvec);
    _mm256_storeu_si256((__m256i*)x,xinew);
    _mm256_storeu_si256((__m256i*)y,yinew);
    x = 32 + (char *) x;
    y = 32 + (char *) y;
    bytes -= 32;
  }
